...
- `portal/tests/`: pytest suite (`test_mapper.py`, `test_eligibility.py`,
  `test_views.py`).

## Build, Test, and Development Commands

- `uv venv && uv pip install -e ".[dev]"`: create environment and install runtime/dev
  dependencies.
...
- Cover both happy paths and edge cases (return window expiry, digital/final-sale items,
...
- Ensure `DJANGO_SETTINGS_MODULE=returns_portal.settings` when running tools outside
  pytest.
